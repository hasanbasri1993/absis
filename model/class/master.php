...
Class Siswa{
	...
	public function getNilaiEkskulx($ekstra_id){
		$dbTA=new DB($_SESSION['database']);

		$sqln ="
		SELECT *
		FROM ekskul_nilai
		WHERE nis=:nis and semester=:semester and ekskul_id=:ekstra_id
		";

		$n = $dbTA->prepare("$sqln");
		$n->bindParam(":nis",$this->_nis,PDO::PARAM_STR);
		$n->bindParam(":semester",$this->_semester,PDO::PARAM_STR);
		$n->bindParam(":ekstra_id",$ekstra_id,PDO::PARAM_STR);
		$n->execute();
		$rown = $n->fetchAll();
	...
	public function getKehadiran(){
		...
		$sqln ="SELECT sakit,izin,alpha FROM kehadiran WHERE nis=:nis and semester=:semester";
	...
	public function setKehadiranS($input){

		$dbTA = new DB($_SESSION['database']);
		$sql = "UPDATE kehadiran SET sakit = :sakit WHERE nis = :nis AND semester = :semester";
		$set = $dbTA -> prepare($sql);
		$set -> bindParam(":sakit", $input);
		$set -> bindParam(":nis", $this -> _nis);
	...
	public function setKehadiranI($input){

		$dbTA = new DB($_SESSION['database']);
		$sql = "UPDATE kehadiran SET izin = :izin WHERE nis = :nis AND semester = :semester";
		$set = $dbTA -> prepare($sql);
		$set -> bindParam(":izin", $input);
		$set -> bindParam(":nis", $this -> _nis);
		$set -> bindParam(":semester", $_SESSION['semester']);
		if ($set -> execute()){
	...
	public function setKehadiranA($input){

		$dbTA = new DB($_SESSION['database']);
		$sql = "UPDATE kehadiran SET alpha = :alpha WHERE nis = :nis AND semester = :semester";
		$set = $dbTA -> prepare($sql);
		$set -> bindParam(":alpha", $input);
		$set -> bindParam(":nis", $this -> _nis);
		$set -> bindParam(":semester", $_SESSION['semester']);
		if ($set -> execute()){
	...
	public function setKehadiran($sakit, $izin, $alpha){

		if (($this -> realExistKehadiran()) == "ok"){

			$this -> setKehadiranS($sakit);
			$this -> setKehadiranI($izin);
			$this -> setKehadiranA($alpha);
		} else {
	...
	public function realExistKehadiran(){

		$ca = $this -> checkExistKehadiran();
		if ($ca == "kosong"){

			if (($this -> insertSiswaKehadiran())){
	...
	public function checkExistKehadiran(){

		$dbTA = new DB($_SESSION['database']);
		$sql = "SELECT nis FROM kehadiran WHERE nis = :nis";
		$check = $dbTA -> prepare($sql);
		$check -> bindParam(":nis", $this -> _nis);
		if ($check -> execute()){
	...
	public function insertSiswaKehadiran(){

		$dbTA = new DB($_SESSION['database']);

		$sql = "INSERT INTO kehadiran (nis, semester) VALUES(:nis, :semester)";

		$insert = $dbTA -> prepare($sql);
		$insert -> bindParam(":nis", $this -> _nis);
		$insert -> bindParam(":semester", $_SESSION['semester']);
		if ($insert -> execute()){
	...
	public function getSikap(){
		...
		$sqln ="SELECT kejujuran, kedisiplinan, tanggungjawab, deskripsi_kejujuran, deskripsi_kedisiplinan, deskripsi_tanggungjawab FROM sikap WHERE nis=:nis and semester=:semester";
	...
	public function setSikapKejujuran($input){

		$dbTA = new DB($_SESSION['database']);
		$sql = "UPDATE sikap SET kejujuran = :kejujuran WHERE nis = :nis AND semester = :semester";
		$set = $dbTA -> prepare($sql);
		$set -> bindParam(":kejujuran", $input);
		$set -> bindParam(":nis", $this -> _nis);
		$set -> bindParam(":semester", $_SESSION['semester']);
		if ($set -> execute()){
	...
	public function setSikapKejujuranDeskripsi($input){

		$dbTA = new DB($_SESSION['database']);
		$sql = "UPDATE sikap SET deskripsi_kejujuran = :deskripsi_kejujuran WHERE nis = :nis AND semester = :semester";
		$set = $dbTA -> prepare($sql);
		$set -> bindParam(":deskripsi_kejujuran", $input);
		$set -> bindParam(":nis", $this -> _nis);
		$set -> bindParam(":semester", $_SESSION['semester']);
		if ($set -> execute()){
	...
	public function setSikapKedisiplinan($input){

		$dbTA = new DB($_SESSION['database']);
		$sql = "UPDATE sikap SET kedisiplinan = :kedisiplinan WHERE nis = :nis AND semester = :semester";
		$set = $dbTA -> prepare($sql);
		$set -> bindParam(":kedisiplinan", $input);
		$set -> bindParam(":nis", $this -> _nis);
		$set -> bindParam(":semester", $_SESSION['semester']);
		if ($set -> execute()){
	...
	public function setSikapKedisiplinanDeskripsi($input){

		$dbTA = new DB($_SESSION['database']);
		$sql = "UPDATE sikap SET deskripsi_kedisiplinan = :deskripsi_kedisiplinan WHERE nis = :nis AND semester = :semester";
		$set = $dbTA -> prepare($sql);
		$set -> bindParam(":deskripsi_kedisiplinan", $input);
		$set -> bindParam(":nis", $this -> _nis);
		$set -> bindParam(":semester", $_SESSION['semester']);
		if ($set -> execute()){
	...
	public function setSikapTanggungJawab($input){

		$dbTA = new DB($_SESSION['database']);
		$sql = "UPDATE sikap SET tanggungjawab = :tanggungjawab WHERE nis = :nis AND semester = :semester";
		$set = $dbTA -> prepare($sql);
		$set -> bindParam(":tanggungjawab", $input);
		$set -> bindParam(":nis", $this -> _nis);
		$set -> bindParam(":semester", $_SESSION['semester']);
		if ($set -> execute()){
	...
	public function setSikapTanggungJawabDeskripsi($input){

		$dbTA = new DB($_SESSION['database']);
		$sql = "UPDATE sikap SET deskripsi_tanggungjawab = :deskripsi_tanggungjawab WHERE nis = :nis AND semester = :semester";
		$set = $dbTA -> prepare($sql);
		$set -> bindParam(":deskripsi_tanggungjawab", $input);
		$set -> bindParam(":nis", $this -> _nis);
		$set -> bindParam(":semester", $_SESSION['semester']);
		if ($set -> execute()){
	...
	public function setSikap($kejujuran, $kedisiplinan, $tanggungjawab, $kejujuran_deskripsi, $kedisiplinan_deskripsi, $tanggungjawab_deskripsi, $deskripsi_deskripsi){

		if (($this -> realExistSikap()) == "ok"){

			$this -> setSikapKejujuran($kejujuran);
			$this -> setSikapKedisiplinan($kedisiplinan);
			$this -> setSikapTanggungJawab($tanggungjawab);
			$this -> setSikapDeskripsi($deskripsi);

			$this -> setSikapKejujuranDeskripsi($kejujuran_deskripsi);
			$this -> setSikapKedisiplinanDeskripsi($kedisiplinan_deskripsi);
			$this -> setSikapTanggungJawabDeskripsi($tanggungjawab_deskripsi);
			$this -> setSikapDeskripsiDeskripsi($deskripsi_deskripsi);
	...
	public function realExistSikap(){

		$ca = $this -> checkExistSikap();
		if ($ca == "kosong"){

			if (($this -> insertSiswaSikap())){
	...
	public function checkExistSikap(){

		$dbTA = new DB($_SESSION['database']);
		$sql = "SELECT nis FROM sikap WHERE nis = :nis AND semester = :semester";
		$check = $dbTA -> prepare($sql);
		$check -> bindParam(":nis", $this -> _nis);
	...
	public function insertSiswaSikap(){

		$dbTA = new DB($_SESSION['database']);

		$sql = "INSERT INTO sikap (nis, semester) VALUES(:nis, :semester)";
	...
	public function getNilaiTotalHarian($kodeNamaMapel){
		...
		if ($this->_kurikulum=="ktsp"){
			...
			*/
			...
			SELECT SUM(UT)
			FROM ktsp_harian
			WHERE ktsp_mapel_id=:ktsp_mapel_id and nis=:nis and semester=:semester and nip=:nip and ut IS NOT NULL and ut != 0
			";
			$n = $dbTA->prepare("$sqln");
			$n->bindParam(":nis",$this->_nis,PDO::PARAM_STR);
			$n->bindParam(":ktsp_mapel_id",$kodeMapel,PDO::PARAM_STR);
			...
			SELECT SUM(UL)
			FROM ktsp_harian
			WHERE ktsp_mapel_id=:ktsp_mapel_id and nis=:nis and semester=:semester and nip=:nip and ul IS NOT NULL and ul != 0
			";
			$n = $dbTA->prepare("$sqln");
			$n->bindParam(":nis",$this->_nis,PDO::PARAM_STR);
			$n->bindParam(":ktsp_mapel_id",$kodeMapel,PDO::PARAM_STR);
			...
			SELECT SUM(t)
			FROM ktsp_harian
			WHERE ktsp_mapel_id=:ktsp_mapel_id and nis=:nis and semester=:semester and nip=:nip and t IS NOT NULL and t != 0
			";
			$n = $dbTA->prepare("$sqln");
			$n->bindParam(":nis",$this->_nis,PDO::PARAM_STR);
			$n->bindParam(":ktsp_mapel_id",$kodeMapel,PDO::PARAM_STR);
			...
			$jumlah_semua = $jumlah_ut[0]+$jumlah_ul[0]+$jumlah_t[0];
	...
	public function getBanyakTotalHarian($kodeNamaMapel){
		...
		if ($this->_kurikulum=="ktsp"){
			...
			*/
			...
			$jumlah_semua = $jumlah_ut+$jumlah_ul+$jumlah_t;
...
Class Mapel{
	...
	public function getRataKelasUTS($kelas,$subkelas,$agama=null){
		if($agama==1){
			...
			while ($count < $banyak_siswa) {
				...
				$totalNilai= $totalNilai + $siswa->getNilaiUTS($kode_mapel);
	     		$count++;
			}

			$rataKelas=$totalNilai/$banyak_siswa;

			return round($rataKelas,0);
			...
			while ($count < $banyak_siswa) {
				$siswa=new Siswa($daftar_siswa[$count]['nis']);
				$totalNilai= $totalNilai + $siswa->getNilaiUTS($this->_kodeNamaMapel);
	     		$count++;
			}
  ...
  public function getRataKelasUAS($kelas,$subkelas){
    $kelasX= new Kelas ($kelas,$subkelas);
    $banyak_siswa= $kelasX->getBanyakSiswa();
    $daftar_siswa= $kelasX->getSiswa();
   ...
   /* if ($this->_kodeNamaMapel=="101"){
    	$banyak_siswa=$kelasX->getBanyakSiswaAgama('HINDU');
    } elseif ($this->_kodeNamaMapel=="102"){
    	$banyak_siswa=$kelasX->getBanyakSiswaAgama('ISLAM');
    } elseif ($this->_kodeNamaMapel=="103"){
    	$banyak_siswa=$kelasX->getBanyakSiswaAgama('KATOLIK');
    } elseif ($this->_kodeNamaMapel=="104"){
    	$banyak_siswa=$kelasX->getBanyakSiswaAgama('KRISTEN');
    } else{
